...
class ScoringEngine:
    ...
    def _score_entity_density(self, parsed: Dict) -> Dict:
        ...
        if self.nlp is not None:
            ...
            doc = self.nlp(text[:100000] if len(text) > 100000 else text)
            entities = [ent.text for ent in doc.ents]
            entity_count = len(set(entities))
        else:
            # Regex fallback: detect proper nouns, organizations, products, locations
            text = parsed["text"]
            entity_patterns = [
                # Capitalized multi-word names (e.g., "Bailey Smith", "Mount Sopris")
                r"\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+\b",
                # Acronyms/initialisms (e.g., CPA, LLC, SEO)
                r"\b[A-Z]{2,6}\b",
                # Common entity suffixes (Inc, LLC, Corp, Ltd, Co)
                r"\b\w+\s+(?:Inc|LLC|Corp|Ltd|Co|Foundation|Institute|University)\b",
                # Product/brand-like patterns (e.g., GPT-4, Claude 3.5)
        ...
        return {
            "score": round(score, 1),
            "max": 15,
            "detected": entities_per_100 >= 1,
            "entity_count": entity_count,
            "entities_per_100": round(entities_per_100, 1),
            "method": "spacy" if self.nlp is not None else "regex",
        }
    ...
    def _score_citation_hooks(self, parsed: Dict) -> Dict:
        ...
        # Text-based citation patterns
        citation_patterns = [
            r"according to\b",
            r"research (?:from|by|at)\b",
            r"(?:study|studies) (?:found|shows?|indicates?|suggests?|published)\b",
            r"(?:data|evidence|findings) (?:from|shows?|suggests?)\b",
            r"source:",
            r"references?:",
            r"cited (?:by|in|from)\b",
            r"published (?:by|in)\b",
            r"reported (?:by|in)\b",
            r"as (?:noted|stated|described|reported) (?:by|in)\b",
        ]
        ...
        citation_count += len(external_links)
    ...
    def _score_recursive_depth(self, parsed: Dict) -> Dict:
        ...
        # Count all questions (sentences ending with ?)
        questions = re.findall(r"[^.!?\n]+\?", text)

        # Question-style headers (strong citability signal)
        question_headers = [h for h in headers if "?" in h["text"]]

        # Detect follow-up/nested structures
        nested_patterns = [
            r"(?:but|however|additionally|furthermore|moreover)\s+(?:what|how|why|when|where|which)",
            r"(?:what|how|why|when|where|which)[^?]*\?[^?]*(?:what|how|why|when|where|which)[^?]*\?",
            r"follow[- ]up",
            r"related(?:ly)?[,:]?\s+(?:what|how|why)",
        ...
        # Score: 15 points max
        question_score = min(5, len(questions) * 0.5)
        header_q_score = min(5, len(question_headers) * 2.5)
        nested_score = min(5, nested_count * 2.5)
        score = question_score + header_q_score + nested_score
